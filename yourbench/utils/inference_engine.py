...
async def _get_response(model: Model, inference_call: InferenceCall) -> str:
    ...
    client = AsyncOpenAI(
        ...
        max_retries=0, # Retries are handled by _retry_with_backoff
    )
    ...
    try:
        completion = await client.chat.completions.create(
            ...
            # Note: Add other parameters like max_tokens if needed based on API/model support
        )
    except OpenAIError as e:
        logger.error("OpenAI API error for model {}: {}", model.model_name, e)
        raise # Reraise the specific error for retry logic
    except Exception as e:
        logger.error("Unexpected error during inference for model {}: {}", model.model_name, e)
        raise # Reraise other potential errors (network issues, etc.)


    # Safe-guarding in case the response is missing .choices or content
    if not completion or not completion.choices or not completion.choices[0].message or completion.choices[0].message.content is None:
        logger.error(
            "Empty response, missing .choices, .message or .content from model {}",
            model.model_name,
        )
        # You might want to return a default value or raise a specific exception
        raise Exception(f"Failed Inference: Invalid response structure from {model.model_name}")
    ...
    for attempt in range(inference_call.max_retries):
        ...
        async with semaphore:  # enforce concurrency limit per-model
            try:
                ...
                logger.error("Error invoking model {} ({}): {}", model.model_name, error_type, e)
