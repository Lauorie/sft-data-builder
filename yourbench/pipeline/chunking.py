...
def _chunk_document_fast_tiktoken(
    ...
    # --- Start Optimization for last chunk handling ---
    # Handle the last remaining chunk
    if current_chunk_tokens: # Check if there are remaining tokens
        if len(current_chunk_tokens) >= l_min_tokens: # If sufficient length, save directly
            chunk_text = tokenizer.decode(current_chunk_tokens)
            chunks_result.append(SingleHopChunk(chunk_id=f"{doc_id}_{chunk_index}", chunk_text=chunk_text))
            # Store the tokens of the last added chunk for potential merging later
            last_saved_chunk_tokens = current_chunk_tokens

        elif chunks_result:  # If the last chunk is too short, try merging with the previous one
            ...
            # --- Optimization: Use previously stored tokens if possible ---
            # We need to track the *tokens* of the last *successfully added* chunk.
            # Let's refine the logic slightly above to store the last added tokens.
            # We need a variable outside the loop or modify how chunks_result stores info.
            # For minimal change now, we'll still re-encode, but acknowledge the optimization point.
            # TODO: Refactor to store last chunk's tokens to avoid this re-encode.
            last_chunk_text = chunks_result[-1].chunk_text
            try:
                 # Attempt re-encoding, acknowledging potential minor differences vs original tokens
                 last_chunk_tokens = tokenizer.encode(last_chunk_text, allowed_special=set())
            except Exception as e:
                 logger.warning(f"Could not re-encode last chunk text for potential merge in doc {doc_id}. Discarding final short chunk. Error: {e}")
                 last_chunk_tokens = None # Flag that merge is not possible

            # Proceed with merge check only if re-encoding succeeded
...
def _multihop_chunking(
    ...
    """
    Creates multi-hop chunks by sampling combinations of single-hop chunks.

    The target number of multi-hop chunks to select is determined by:
        num_multihops = max(1, total_single_hops // num_multihops_factor).

    It generates combinations of sizes ranging from h_min to h_max.
    Instead of generating ALL possible combinations (which can be huge), it shuffles
    the single-hop chunk indices and generates combinations based on the shuffled order
    until the target number (num_multihops) is reached or all possible combinations
    of the requested sizes have been generated. This is much more efficient if the
    number of single hops is large and num_multihops_factor leads to a smaller target.
    ...
    # Ensure h_min and h_max are valid relative to the number of chunks
    total_single_hops = len(single_hop_chunks)
    h_min = min(h_min, total_single_hops) # Cannot be larger than total chunks
    h_max = min(h_max, total_single_hops) # Cannot be larger than total chunks

    if h_min > h_max or h_min <= 0:
         logger.warning(f"Invalid h_min/h_max ({h_min}/{h_max}) for {total_single_hops} single hops. Skipping multi-hop.")
         return []

    # This is our target count for how many multi-hop combos we want to keep
    num_multihops = max(1, total_single_hops // num_multihops_factor)

    # Optimization: Instead of generating all combinations, sample them.
    # We shuffle the indices to get random combinations.
    indices = list(range(total_single_hops))
    random.shuffle(indices) # Shuffle indices in place

    sampled_combos: list[MultiHopChunk] = []
    generated_combo_indices = set() # Keep track of generated index combinations to ensure uniqueness

    # Iterate through combination sizes
    for size in range(h_min, h_max + 1):
        # Generate combinations from the *shuffled* indices
        # Use itertools.combinations on the shuffled list
        for combo_indices_tuple in itertools.combinations(indices, size):
             # Sort the tuple to make the order consistent for the uniqueness check
             sorted_combo_indices = tuple(sorted(combo_indices_tuple))

             if sorted_combo_indices not in generated_combo_indices:
                 chosen_chunks = [single_hop_chunks[idx] for idx in sorted_combo_indices]
                 group_obj = MultiHopChunk(
                     chunk_ids=[c.chunk_id for c in chosen_chunks],
                     chunks_text=[c.chunk_text for c in chosen_chunks],
                 )
                 sampled_combos.append(group_obj)
                 generated_combo_indices.add(sorted_combo_indices)

                 # Stop if we have enough samples
                 if len(sampled_combos) >= num_multihops:
                     return sampled_combos[:num_multihops] # Return exactly num_multihops

    # If we finish iterating and haven't reached num_multihops, return all unique ones found
...
def _compute_info_density_metrics(
    ...
    for chunk in chunks:
        ...
        if local_perplexity_metric is not None and chunk_text.strip():
            try:
                # Add a warning about performance impact here, maybe only once?
                # Let's add it outside the loop if the metric is enabled.
                # (See addition near the function call)
                # logger.debug("Computing perplexity - this can be slow...") # Example inline log
