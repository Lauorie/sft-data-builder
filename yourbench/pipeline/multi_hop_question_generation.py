...
from yourbench.utils.prompts import (
    ...
    MULTI_HOP_QUESTION_GENERATION_USER_PROMPT_ZH,
    MULTI_HOP_QUESTION_GENERATION_SYSTEM_PROMPT_ZH,
)
...
class MultiHopQuestionRow:
    ...
    source_chunk_texts: List[str] = field(default_factory=list)
    ...
    def from_qa_pair(
        ...
        raw_response: str = "",
        additional_instructions: str = "",
        source_chunk_texts: List[str] = None,
    ) -> "MultiHopQuestionRow":
        if source_chunk_texts is None:
            source_chunk_texts = []
        return cls(
...
def _multihop_chunk_sampling_and_calls(dataset, stage_cfg: Dict[str, Any]):
    ...
    Returns:
      ...
      - call_index_map: parallel list of (row_idx, document_id, document_filename, source_chunk_ids, source_chunk_texts)
    """
    system_msg = {
        "role": "system",
        "content": MULTI_HOP_QUESTION_GENERATION_SYSTEM_PROMPT_ZH,
    }
    all_inference_calls = []
    call_index_map = []
    ...
    for row_idx, row in enumerate(dataset):
        ...
        for mh_idx, mh_dict in enumerate(chosen_multi_hops):
            ...
            user_prompt_str = MULTI_HOP_QUESTION_GENERATION_USER_PROMPT_ZH.format(
                title=title,
            ...
            call_index_map.append((row_idx, doc_id, title, subchunk_ids, subchunk_texts))
...
def _parse_and_build_final(
    ...
    for model_name, model_responses in responses_dict.items():
        ...
        for idx, raw_resp in enumerate(model_responses):
            ...
            row_idx, doc_id, doc_filename, source_chunk_ids, source_chunk_texts = call_index_map[idx]
            ...
            for qap_dict in qa_pairs:
                try:
                    if not isinstance(qap_dict, dict):
                        logger.warning(f"Skipping invalid QA pair item (not a dict): {qap_dict} for doc_id={doc_id}")
                        continue
                    ...
                    logger.warning(f"Error processing QA pair item {qap_dict} for doc_id={doc_id}, skipping pair: {pair_error}")
                    continue
